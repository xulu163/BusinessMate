$(function() {
	...
	$("#exportExcel_wait").click(function() {
		console.info("exportExcel_wait--export");
		var $postJson = {};
		$postJson.startDate = $('#fbeginDate').val();
		$postJson.endDate = $('#fendDate').val();
		var url = "/api/contracts/wait/excel/" + JSON.stringify($postJson);
		window.location = url;
	});
		...
		$.ajax({
			...
			success: function(data) {
				...
				$.each(userResources, function(i, itemData) {
					...
					if (url == "/api/contracts/wait/excel/:keywords") {
						console.info(url);
						if (oper == "get") {
							excelFlag = 1;
						}
				...
				if (excelFlag == 0) {
					$("#exportExcel_wait").css('display', 'none');
				}
	...
	var initialize = function(cpage, start, end, sortType, order) {
		...
		$.ajax({
			url: '/api/contracts/tasks/wait/' + JSON.stringify($postJson),
			type: 'get',
			// data: data,
			success: function(data) {
				if (!showPri(data)) {
	...
	var serializeData = function(data) {
		...
		$.each(data, function(i, itemData) {
			...
			s2 = itemData.waitAmount + "";
			itemData.waitAmount = "￥" + s2.replace(/\B(?=(?:\d{3})+$)/g, ',');
			...
			var tempData = {
				...
				waitAmount: itemData.waitAmount,
			...
			var template = "<tr><td class=center title='" + itemData.contractName + "'><a href='/contracts/" + itemData._id + "/edit'>{{name}}</a></td><td class='hidden-480 center'>{{partyAabbr}}</td><td class='hidden-480 center'>{{partyBabbr}}</td><td class='center'>{{amount}}</td><td class=center>{{waitAmount}}</td><td class='hidden-phone center'>" + easypieRatio + "</td><td class=center>{{priceDate}}</td><td class='hidden-phone center'>{{date}}</td><td class='center hidden-480'>{{state}}</td></tr>"

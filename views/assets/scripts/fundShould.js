$(function() {
	...
	/*对数据进行前台排序*/
	var sortType = "priceDate"; //默认按日期排序
	var order = "asc"; //默认倒序排列
	...
	$("#exportExcel").click(function() {
		console.info("funshould--export");
		var $postJson = {};
		$postJson.startDate = $('#fbeginDate').val();
		$postJson.endDate = $('#fendDate').val();

		var url = "/api/contracts/should/excel/" + JSON.stringify($postJson);
		window.location = url;
		...
		$.ajax({
			...
			success: function(data) {
				...
				$.each(userResources, function(i, itemData) {
					...
					if (url == "/api/contracts/should/excel/:keywords") {
						console.info(url);
						if (oper == "get") {
							excelFlag = 1;
						}
	...
	var initialize = function(cpage, initStart, initEnd, sortType, order) {
		authority();
		...
		$postJson.bDate = initStart;
		$postJson.eDate = initEnd;
		$postJson.sortType = sortType;
		$postJson.order = order;

		$.ajax({
			url: '/api/contracts/tasks/unreturn/' + JSON.stringify($postJson),
			type: 'get',
			// data: data,
			success: function(data) {
				if (!showPri(data)) {
				...
				var options = {
					...
					onPageChanged: function(e, oldPage, newPage) {
						initialize(newPage, initStart, initEnd, sortType, order);
					}
				};
	...
	var serializeData = function(data) {
		...
		$.each(data, function(i, itemData) {
			...
			s2 = itemData.unreturnAmount + "";
			itemData.unreturnAmount = "￥" + s2.replace(/\B(?=(?:\d{3})+$)/g, ',');
			var tempData = {
				name: contractName,
				partyAabbr: itemData.partyAabbr,
				partyBabbr: itemData.partyBabbr,
				amount: itemData.amount,
				unreturnAmount: itemData.unreturnAmount,
			...
			var template = "<tr><td class=center title='" + itemData.contractName + "'><a href='/contracts/" + itemData._id + "/edit'>{{name}}</a></td>" +
				"<td class='hidden-480 center'>{{partyAabbr}}</td>" +
				"<td class='hidden-480 center'>{{partyBabbr}}</td>" +
				"<td class='center'>{{amount}}</td>" +
				"<td class=center>{{unreturnAmount}}</td>" +
				"<td class='hidden-phone center'>" + easypieRatio + "</td>" +
				"<td class=center>{{priceDate}}</td>" +
				"<td class='center hidden-480'>{{state}}</td></tr>"
